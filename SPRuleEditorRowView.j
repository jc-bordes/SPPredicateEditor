...
@implementation SPRuleEditorRowView : CPView
{
	...
	SPRuleEditorModelItem 	_item @accessors(property=item);
...
//TODO Use theme image
	...
	_alternateAddButtonImage=[[CPImage alloc] initWithContentsOfFile:[[CPBundle bundleForClass:[SPRuleEditor class]] pathForResource:@"rule-editor-add-compound.png"] ];
...
-(CPButton)createButtonWithAction:(SEL)action image:(CPImage)image atPosition:(CPInteger)position
{
    ...
    var button=[[SPRuleEditorActionButton alloc] initWithFrame:buttonFrame];
...
-(void)setItem:(SPRuleEditorModelItem)item
{	
	[self resetCriteria];
	...
	if(count==0)
		[CPException raise:CPInternalInconsistencyException reason:_cmd+@" : Empty SPRuleEditorCriterion"];
...
-(void)setDelegate:(id)delegate
{
    ...
    if(_delegate)
    {
		[notificationCenter removeObserver:self name:SPRuleEditorViewAltKeyDown object:_delegate];
		[notificationCenter removeObserver:self name:SPRuleEditorViewAltKeyUp object:_delegate];
    }
    
	_delegate=delegate;
    
    if(_delegate&&[_delegate nestingMode]==SPRuleEditorNestingModeCompound)
    {
		[notificationCenter addObserver:self selector:@selector(altKeyDown:) name:SPRuleEditorViewAltKeyDown object:_delegate];
		[notificationCenter addObserver:self selector:@selector(altKeyUp:) name:SPRuleEditorViewAltKeyUp object:_delegate];
...
-(CPView)addViewForCriterion:(SPRuleEditorCriterion)criterion withValue:(id)value atIndex:(CPInteger)index afterCriterionView:(SPRuleEditorCriterion)previousCriterion
{
	if(![criterion isStandaloneView])
		[CPException raise:CPInternalInconsistencyException reason:_cmd+@" : Invalid SPRuleEditorCriterion"];
...
-(CPView)addMenuForCriterion:(SPRuleEditorCriterion)criterion withValue:(id)value atIndex:(CPInteger)index afterCriterionView:(SPRuleEditorCriterion)previousCriterion
{
	if(![criterion isMenu])
		[CPException raise:CPInternalInconsistencyException reason:_cmd+@" : Invalid SPRuleEditorCriterion"];
    ...
    var popup=[[SPRuleEditorPopUpButton alloc] initWithFrame:frame];
    [popup setValue:font forThemeAttribute:@"font"];
    [popup setValue:CGSizeMake(0.0, SPRuleEditorRowViewCriterionHeight) forThemeAttribute:@"min-size"];
...
-(CPView)addStaticTextForCriterion:(SPRuleEditorCriterion)criterion afterCriterionView:(SPRuleEditorCriterion)previousCriterion
{
	if(![criterion isMenu])
		[CPException raise:CPInternalInconsistencyException reason:_cmd+@" : Invalid SPRuleEditorCriterion"];
...
-(void)addRowView:(SPRuleEditorRowView)aView
{
	[_subrowsView addSubview:aView];
}

-(void)removeRowView:(SPRuleEditorRowView)aView
{
	[aView removeFromSuperview];		
}

-(void)addRowView:(SPRuleEditorRowView)rowView positioned:(CPInteger)position relativeTo:(CPView)aView
{
...
-(void)_layoutSubviews
{
    ...
    [_addButton setHidden:(!_editable||(_delegate&&[_delegate nestingMode]==SPRuleEditorNestingModeSingle))];
...
- (void)drawBordersInContext:(CGContext)context verticalOffset:(float)vOffset width:(float)width
{
	...
	{
		subrow=subrows[i];
		if(![subrow isKindOfClass:SPRuleEditorRowView])
			continue;
...
-(void)addClicked:(id)sender
{
	...
	var rowType=(altKeyPressed&&_delegate&&[_delegate nestingMode]==SPRuleEditorNestingModeCompound)?SPRuleEditorRowTypeCompound:SPRuleEditorRowTypeSimple;
...
-(BOOL)hasSubrow:(SPRuleEditorRowView)rowView
{
...
- (void)mouseDragged:(CPEvent)event
{
    ...
    [pasteboard declareTypes:[CPArray arrayWithObjects: SPRuleEditorItemPBoardType, nil] owner: self];
...
-(void)flagsChanged:(CPEvent)anEvent
{
    ...
    if(_delegate&&[_delegate nestingMode]==SPRuleEditorNestingModeCompound)
	{
		...
		{
			[[CPNotificationCenter defaultCenter] postNotificationName:SPRuleEditorViewAltKeyDown object:_delegate userInfo:nil];
		}
		else
		{
			[[CPNotificationCenter defaultCenter] postNotificationName:SPRuleEditorViewAltKeyUp object:_delegate userInfo:nil];
		}
	}
}
...
@implementation CPTextField(SPRuleEditorRowView)
